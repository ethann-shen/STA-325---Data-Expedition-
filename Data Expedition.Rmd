...
```{r}
lm.fit.2 <- lm(log_second_moment ~ St + Re + Fr + Re*Fr, data=trainSecondMomentFct)
summary(lm.fit.2)
```


```{r}
cv.lm(trainSecondMomentFct, form.lm = formula(log_second_moment ~ St + Re + Fr + Re*Fr), m = 5)
```

### Polynomial

```{r}
cv2 <- cv.polywog(log_second_moment ~ St + Re + Fr,
                  data = trainSecondMomentFct,
                  degrees.cv = 1:4,
                  nfolds = 5,
                  thresh = 1e-4)
print(cv2)
...
```{r}
lm.fit.3.1 <- lm(R_moment_3 ~ St + Re + Fr, data = train.3) 
summary(lm.fit.3.1)
cv.lm(train.3, form.lm = formula(R_moment_3 ~ St + Re + Fr + Re*Fr), m = 5)
```

```{r}
p <- ggplot(data=train.3, aes(x=Re,y=log_third_moment, color=Fr)) + geom_smooth()
p
```

Initial data exploration in the introduction showcased a long right tail, suggesting that a long transformation on the response to be appropriate. The above plot which shows co-movement between Re and Fr suggests that an interaction between the two variables could be appropriate. Let's improve our baseline linear model with these enhancements.

```{r}
train.3.1 <- train.3 %>%
  mutate(log_third_moment = log(R_moment_3))
lm.fit.3.2 <- lm(log_third_moment ~ St + Re + Fr + Re*Fr, data=train.3.1)
summary(lm.fit.3.2)
cv.lm(train.3.1, form.lm = formula(log_third_moment ~ St + Re + Fr + Re*Fr), m = 5)
```
...
```{r}
cv1 <- cv.polywog(log_third_moment ~ St + Re + Fr,
                  data = train.3.1,
                  degrees.cv = 1:4,
                  nfolds = 5,
                  thresh = 1e-4)
...
Here, we explore a polynomial fit. Through bootstrapping, the polywog package selected degree 2. This is an improvement on the baseline linear model, but not on the 
...
data$Fr <- invlogit(data$Fr)

split <- sample(1:nrow(data), nrow(data) * 0.8) # 50% split
train.data <- data[split, ]
test.data <- data[-split,]
```

```{r}
lm.fit.4 <- lm(R_moment_4 ~ St + Re + Fr, data = train.data) 
summary(lm.fit.4)
library(DAAG)
cv.lm(train, form.lm = formula(R_moment_4 ~ St + Re + Fr), m = 5)
```
...
#GOOD MODEL 
library(mgcv)
gam.3 <- gam(log(R_moment_4) ~ s(log(St)) + bs(Re) + bs(Fr) + bs(Re * Fr), data = train)
summary(gam.3)
AIC(gam.3)
...
```{r}
library(gamclass)
gamclass::CVgam(log(R_moment_4) ~ s(log(St)) + bs(Re) + bs(Fr) + bs(Re * Fr), data = train, nfold = 5, method = "GCV.Cp")
```
...
```{r}
spline.3 <- lm(log(R_moment_4) ~ ns(St) + ns(Re) + ns(Fr), data = train) #.4697, AIC 4410.8
spline.4 <- lm(log(R_moment_4) ~ bs(St) + ns(Re) + ns(Fr), data = train) #.4607

#BEST CV w intrpretable model lmfao
gam.cv.4 <- gamclass::CVgam(log(R_moment_4) ~ bs(St) + ns(Re) + ns(Fr), data = train, nfold = 5, method = "GCV.Cp")
...
gamclass::CVgam(log(R_moment_4) ~ ns(St) + bs(Re) + ns(Fr), data = train, nfold = 5, method = "GCV.Cp")
#BEST CV
gamclass::CVgam(log(R_moment_4) ~ ns(St) + bs(Re) + ns(Fr), data = train, nfold = 5, method = "GCV.Cp")

summary(lm(log(R_moment_4) ~ ns(St) + ns(Re + Fr), data = train))
...
# library(gbm)
# set.seed(1)
# bleh <- gbm(R_moment_4 ~ St + Fr + Re, data = train, 
#             distribution = "gaussian", n.trees = 5000)
# 
# summary(bleh)
# library(adabag)
# library(gbm)
# boosting.cv(R_moment_4 ~ St + Fr + Re, train, v = 10, boos = TRUE, mfinal = 100, 
#             coeflearn = "Breiman", control, par=FALSE)
# 
# gbmCrossVal(cv.folds=10, train,  x, y,
#             offset, distribution, w, var.monotone, n.trees, interaction.depth,
#             n.minobsinnode, shrinkage, bag.fraction, var.names, response.name, group)
```
