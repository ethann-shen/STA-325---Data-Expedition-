...
```{r}
lm.fit.4 <- lm(log(R_moment_4) ~ St + Re + Fr, data = train.data) 
summary(lm.fit.4)
library(DAAG)
...
```{r}
spline.3 <- lm(log(R_moment_4) ~ ns(St + Re) + ns(Fr), data = train) #.4697, AIC 4410.8
...
smooth.spline(train$R_moment_4, train$St, train$Re, train$Fr, cv = TRUE)
