...
```{r}
gam.fit.3.1 <- gam(log_third_moment~ s(St) + Re + Fr, data=train.3.1)
gamclass::CVgam(log_third_moment ~ s(log(St)) + Re + Fr, data = train.3.1, nfold = 5, method = "GCV.Cp")
AIC(gam.fit.3.1)
summary(gam.fit.3.1)
```
...
```{r}
gam.fit.3.2 <- gam(log_third_moment~ s(St) + Re + Fr + Re:Fr, data=train.3.1)
AIC(gam.fit.3.2)
summary(gam.fit.3.2)
gamclass::CVgam(log_third_moment ~ s(log(St)) + Re + Fr + Re * Fr, data = train.3.1, nfold = 5, method = "GCV.Cp")
```
...
```{r}
spline.fit.3a <- lm(log_third_moment ~ ns(log(St) + Re) + ns(Fr), data = train.3.1) 
spline.fit.3b <- lm(log_third_moment ~ ns(St + Re) + bs(Fr), data = train.3.1) 
```
