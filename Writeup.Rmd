...
We are using the linear model as our final model for moment 1. It has the lowest CV error and AIC. 
...
lm.fit.a <- lm(log_first_moment ~ St + log(Re) + log(Fr), data = train)
a <- train %>%
  mutate(residuals = resid(lm.fit.a))
...
m1.diag <- plot_grid(title_1, plot_grid(a1, a2, a3, a4), ncol = 1, rel_heights = c(0.1,1))
```
...
We are using the linear model as our final model for moment 2. Although it does not have the lowest error values, it is more interpretable than a GAM. 
...
m2.diag <- plot_grid(title_2, plot_grid(b1, b2, b3, b4), ncol = 1, rel_heights = c(0.1,1))
```
...
m3.diag <- plot_grid(title_3, plot_grid(c1, c2, c3, c4), ncol = 1, rel_heights = c(0.1,1))
```
...
We are using the GAM with interactions as our final model for moment 4. Even though the error is lower for polynomial model, it is more interpretable, especially when we are trying to consider the impact of multiple variables. 
...
m4.diag <- plot_grid(title_4, plot_grid(d1, d2, d3, d4), ncol = 1, rel_heights = c(0.1,1))
```
...
Across all four moments, St has a positive impact on the outcome prediction. Re and Fr have a negative impact on the outcome prediction. Judging from p-values, Re is consistently the most important predictor, and the interaction between these two parameters is important, especially for high moments. 

--INTERACTION INTERPRETATION HERE--
